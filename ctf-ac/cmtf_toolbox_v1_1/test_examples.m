...
data = tester_acmtf('R',3,'size',[30 20 10 40], 'lambdas',{[1 1 1],[0 1 1]}, 'modes',{[1 2 3],[1 4]},'beta_cp',0.001, 'beta_pca',0.001);
...
data = tester_cmtf_missing('size',[10,20,30,20,30,20], 'modes', {[1 2 3 4], [1 2 5 6]}, 'R', 4, 'M', [0.6 0.2]);
...
norm(trueval-estval)/norm(trueval)


plot(trueval,estval,'r.');xlabel('True Values');ylabel('Estimated Values'); title('Missing Data Estimation');
...
data = tester_acmtf_missing('size',[40 40 40 40], 'modes', {[1 2 3], [1 4]}, 'R', 4, 'M', [0.4 0],'flag_sparse',[0 0],'lambdas',{[1 1 1 1],[1 1 1 1]},'beta_cp',0.001, 'beta_pca',0.001);

% Compute the error between the true values of missing entries and the estimated values.
trueval = data.Xorig{1}(find(data.W{1}==0));
Z       = full(data.Zhat{1});
estval  = Z(find(data.W{1}==0));
sqrt(immse(trueval,estval))
