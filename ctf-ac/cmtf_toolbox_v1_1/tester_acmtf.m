function  data = tester_acmtf(varargin)
...
%
%   data = tester_acmtf('beta_cp',...,'beta_pca') gives as input the sparsity
%   penalty parameters.
%
...
params.addParamValue('lambdas', {[1 1 1], [1 1 0]}, @iscell);
...
if any(flag_sparse)
    ...
    [X, Atrue] = create_coupled_sparse('size',sz,'modes',modes,'lambdas',lambdas,'flag_sparse',flag_sparse);
else
...
% fit ACMTF-OPT
[Zhat,G,out]    = acmtf_opt(Z,R,'init',init,'alg_options',options, 'beta_cp', beta_cp, 'beta_pca', beta_pca);        
data.Zhat       = Zhat;
data.out        = out;
data.Atrue      = Atrue;
l_rec = zeros(P, R);
